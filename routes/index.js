...
router.post('/hotels/:id/:name/booking-confirmation', function (req, res) {
  ...
  req.pool.getConnection(function(err, connection){
    if(err) throw err;
      var sql = "INSERT INTO bookings VALUES ('"+bookingID+"', '"+req.session.username+"', '"+thisRoom.hotel_id+"', '"+thisRoom.room_id+"' '"+req.body.from+"', '"+req.body.to+"', '"+thisHotel.hotel_name+"', '"+thisRoom.room_name+"');";
      connection.query(sql, function(err, results){
        connection.release();
        console.log(results);
    });
    //incrementing the ID of the next booking
    bookingID++;
